...
## Fase 8 — Sprint 8: Safe Area Notch, Realtime Leaderboard, Debounce Reactivo y Onboarding Cultural Inmersivo

| Fecha | Autor | Componente / Archivo(s) | Hito alcanzado | Pruebas ejecutadas |
| :--- | :--- | :--- | :--- | :--- |
| 2026-09-26 | Equipo Yachay (asistencia Claude Sonnet 4.6) | `app/_layout.tsx`, `components/yachay/yachay-top-bar.tsx` | **Safe Area Notch Fix:** `SafeAreaProvider` envuelve el árbol completo en `_layout.tsx`. `YachayTopBar` usa `SafeAreaView edges={['top']}` para ajustar automáticamente el paddingTop con el inset real del notch/isla dinámica en todos los dispositivos. | Verificación visual en iOS con Dynamic Island y Android con punch-hole camera. |
| 2026-09-26 | Alejandro Padilla (asistencia Claude Sonnet 4.6) | `app/(tabs)/index.tsx` | **useSafeAreaInsets en index.tsx:** el toast de nodo bloqueado calcula su `bottom` dinámicamente con `useSafeAreaInsets` vía `StyleSheet.create` en `useMemo`, evitando solapamiento con el indicador de inicio (iPhone X+). | `tsc --noEmit` → 0 errores. `grep "style={{"` → 0 hallazgos (INV-02). |
| 2026-09-26 | Alejandro Padilla (asistencia Claude Sonnet 4.6) | `src/context/GameContext.tsx` | **Debounce reactivo 800 ms + upsert inmediato en leaderboard:** La función `addXp` dispara `leaderboardService.syncUserTotalXp()` de forma inmediata (sin debounce) para cada ganancia de XP. El sync debounced a 800 ms (`authService.updateGameState`) sigue activo como capa de persistencia completa para `profiles`. | `__tests__/GameContext.test.tsx` 6/6 PASS con mock de `leaderboardService`. |
| 2026-09-26 | Oscar Segovia (asistencia Claude Sonnet 4.6) | `src/services/leaderboardService.ts`, `app/(tabs)/leaderboard.tsx` | **Realtime Leaderboard:** `subscribeToLeaderboardChanges()` crea un canal Supabase Realtime (`postgres_changes` en `leaderboard_weekly`); `leaderboard.tsx` suscribe con patrón `useRef` de callback estable (evita stale closures) y limpia el canal al desmontar. | Verificación manual de actualización automática del ranking cuando otro usuario suma XP. |
| 2026-09-26 | Yesica Escobar (asistencia Claude Sonnet 4.6) | `app/blocked.tsx`, `app/(tabs)/profile.tsx`, `app/certificate/index.tsx`, `app/(tabs)/shop.tsx`, `app/(tabs)/leaderboard.tsx` | **INV-04 100%:** Todos los `Alert.alert` y `window.alert` reemplazados por componentes visuales animados: toast auto-dismiss, banners de feedback con color semántico (rojo error/verde éxito), y modal de confirmación personalizado para cierre de sesión. | `grep -rn "alert(" app/` → 0 hallazgos (INV-04). |
| 2026-09-26 | Equipo Yachay (asistencia Claude Sonnet 4.6) | `app/onboarding/index.tsx` | **Onboarding Inmersivo Cultural v2.0 (6 pasos):** Paso 0 rediseñado con `yachiPrincipal` (INV-06 corregido), globo de diálogo pedagógico "¡Allillanchu!", tarjeta de análisis morfológico-etimológico (`YACHA- + -Y → YACHAY = "aprender · sabiduría"` con ejemplo), y los 3 pilares andinos (Yachay/Munay/Llank'ay). **Nuevo Paso 1 `StepLinguistic`:** Yachi (`yachiPiensa`) con burbujas de vocales A·I·U, 3 tarjetas sobre Runa Simi / variantes regionales / norma Chanka+Cusco-Collao, y banner del sistema trivocálico oficial. Total: 6 pasos, `Math.min(s+1, 5)`. | Flujo de onboarding de paso 0 a 5 verificado. `tsc --noEmit` → 0 errores. INV-02, INV-06 limpios. |
| 2026-09-26 | Oscar Segovia (asistencia Claude Sonnet 4.6) | `app/modal.tsx` | **Modal Créditos e Identidad Cultural v2.0:** Import migrado de `constants/illustrations` a `src/assets/images` (INV-06). Nueva sección "Variante Lingüística Enseñada" con 3 píldoras: norma oficial, sistema trivocálico y cobertura geográfica. Versión actualizada a 2.0. | Verificación de apertura del modal desde perfil; `grep "supabase\." app/` → 0 hallazgos (INV-03). |
| 2026-09-26 | Yesica Escobar (asistencia Claude Sonnet 4.6) | `app/(tabs)/profile.tsx`, `__tests__/GameContext.test.tsx` | Botón "Créditos e Identidad Cultural" en Perfil (texto actualizado con referencia explícita a etimología y Runasimi). Mock de `leaderboardService` añadido a `GameContext.test.tsx` para aislar WebSocket de Supabase en entorno Jest (Node.js 18). Suite: **20/20 PASS**, 0 errores TypeScript, triple grep limpio. | `docker compose exec expo-web npm test` → 20/20 PASS. `tsc --noEmit` → 0. |
